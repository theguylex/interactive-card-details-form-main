document.addEventListener('DOMContentLoaded', () => {
    ...
    function validateCardholder(value) {
        ...
        if (!value) {
            ...
            showError(cardholderInput, errorMsg, 'Must contain first and last name, no numbers');
            return false;
        } else {
    ...
    button.addEventListener('click', (e) => {
        ...
        const isCardNumberValid = validateCardNumber(cardNumberInput.value.replace(/\D/g, ''));
        const isExpDateValid = validateExpDate();
        const isCVCValid = validateCVC(cvcInput.value);

        if (isCardholderValid && isCardNumberValid && isExpDateValid && isCVCValid) {
